...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    private bool isEnablePreviewBT;
    private int count;
    public static ScrollSnapRect _instance;
    public Transform[] PanelChild;
    ...
    private void Start()
    {
        ...
        //}
        //  count = PlayerPrefs.GetInt("CountPage");
        //  PlayerPrefs.DeleteKey("CountPage");
        _lerp = false;
        //   PlayerPrefs.SetInt("CountPage", count);
    ...
    {
        foreach (GameObject t in page)
        {
            t.SetActive(false);
        }
    }
    //all check button Next will disable if not current pag and page + 1
    private void DisablePAge()
    {
        for (int i = 0; i < page.Length; i++)
        {
            // disable all page after
            if (i + 1 < _currentPage)
            {
                page[i].SetActive(false);
            }
            //all enable next page 
            if (i - 1 >= _currentPage)
            {
                page[_currentPage].SetActive(true);
            }
            // set page 11 enable because limit page
            if (_currentPage == 11)
            {
    ...
    //deactive preview page
    private void DisablePAgePRe()
    {
        for (int i = 0; i < page.Length; i++)
        {
            // disable befor page preview -1
            if (i - 1 > _currentPage)
            {
                page[i].SetActive(false);
            }
    ...
    {
        if (PlayerPrefs.GetInt("ValueMaxLevel") >= 0)
        {
            count = 0;
            startingPage = 0;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 12)
        {
            startingPage = 1;
            count = 1;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 24)
        {
            startingPage = 2;
            count = 2;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 36)
        {
            startingPage = 3;
            count = 3;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 48)
        {
            startingPage = 4;
            count = 4;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 60)
        {
            startingPage = 5;
            count = 5;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 72)
        {
            startingPage = 6;
            count = 6;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 84)
        {
            startingPage = 7;
            count = 7;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 96)
        {
            startingPage = 8;
            count = 8;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 108)
        {
            startingPage = 9;
            count = 9;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 120)
        {
            startingPage = 10;
            count = 10;
        }
        if (PlayerPrefs.GetInt("ValueMaxLevel") > 132)
        {
            startingPage = 11;
            count = 11;
        }
        page[startingPage].SetActive(true);
        for (int i = 0; i < page.Length; i++)
        {
            //    i = _currentPage;
            //if (_currentPage  == startingPage)
            //{
            //    page[i].SetActive(false);
            //}
        }
